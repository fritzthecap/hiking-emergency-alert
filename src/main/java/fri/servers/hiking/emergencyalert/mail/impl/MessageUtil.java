...
public final class MessageUtil
{
    ...
    public static String textContent(Part part) throws  IOException, MessagingException {
        final String textContent = textContent(part, false); // first try text/plain
        // HTML-only mails would deliver empty text now!
        return (textContent != null && textContent.length() > 0)
            ? textContent
            : textContent(part, true); // try to find text/html
    }
