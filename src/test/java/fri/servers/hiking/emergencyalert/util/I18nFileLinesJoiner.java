...
public class I18nFileLinesJoiner
{
    ...
    public void join(String keyFile, String valueFile) throws IOException {
        ...
        for (int i = 0; i < keyLines.size(); i++) {
            ...
            if (key.contains(" "))
                throw new IllegalArgumentException("A key must not contain spaces: >"+key+"<, found in "+keyFile);
            
            String value = valueLines.get(i);
            if (value == null)
                throw new IllegalArgumentException("Found no value for key '"+key+"' in "+valueFile);
