...
You would get an activation mail immedately after activation of the hike (or, for remote activation, one polling-interval afterwards). If you respond to it, no alert mails would be sent ever. If not, alerts would be sent after the planned home-time to all contacts (that would be you in this test-case). Mind that a "passing-to-next" mail would never be sent to a contact that has the same mail address as the next contact!
